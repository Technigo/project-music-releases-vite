    ...
    return (
        <>
            {artists.map((artist, index) => ( // Passing in the index here to check for inside the map
                <a className="artist-name" href={artistUrl.spotify}  key={artist.id}>{ (index ? ', ': '') + artist.name }</a> // If index exists it will return comma+space, if it doesn't exist then Index is 0, and "nothing" is returned
            ))}
        </>
    );
}
